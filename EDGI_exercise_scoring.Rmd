...
## loads raw data and scoresheets

EDGI_raw <- read_sas('exercise.sas7bdat') 
...
ED100k_scored <- ED100k_scored |> 
  mutate (currentbmi = na_if(currentbmi, expss::gt(60))) |> # outlier BMI values
  mutate (currentbmi = na_if(currentbmi, expss::lt(10))) |> 
  mutate (highestbmi = round(703*wt_hi_lb/(height^2), 2))  |> 
  mutate (highestbmi = na_if(highestbmi, expss::gt(60))) |>
  mutate (highestbmi = na_if(highestbmi, expss::lt(10))) |> 
  mutate (lowestbmi = round(703*wt_lo_lb/(height^2), 2)) |> 
  mutate (lowestbmi = na_if(lowestbmi, expss::gt(60))) |> 
  mutate (lowestbmi = na_if(lowestbmi, expss::lt(10)))  |> 
...
save (ED100k_scored, file = "EDGI_scored_data.RData")
```
...
resave (EDEQ_scored, file = "EDGI_scored_data.RData")
...
resave (CET_scored, file = "EDGI_scored_data.RData")

```

```{r warning = FALSE, echo=FALSE}
load('EDGI_scored_data.RData')
...
write_sas(EDGI_exercise_cleaned, 'exercise_cleaned.sas7bdat')
save(EDGI_exercise_cleaned, file = "EDGI_exercise_cleaned.RData")
```
