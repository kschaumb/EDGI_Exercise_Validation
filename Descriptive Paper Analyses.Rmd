...
## Gender
```{r}
ct <- table(EDGI_exercise_cleaned$sex, EDGI_exercise_cleaned$ED100k_exercise_icb_factor)

sex_row_percents <- prop.table (ct, 1)

sex_row_percents <- as.data.frame(sex_row_percents) |> 
 rename(`Sex` = Var1) |> 
 rename(`History of Driven Exercise` =Var2) 

ggplot(sex_row_percents, aes( x = `History of Driven Exercise`, y = Freq*100, fill = Sex )) + 
  geom_col()+
  facet_wrap (~ Sex) +
  labs(title = 'History of Driven Exercise across Sex (assigned at birth)', x = 'History of Driven Exercise', y = 'Percentage (within Sex)') + 
  scale_x_discrete(labels = c('Never', 'A few', 'More Often'))
```
