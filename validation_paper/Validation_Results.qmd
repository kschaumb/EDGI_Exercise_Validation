...
heatmap_df <- rbind(USA_results$Heatmap_df, SE_results$Heatmap_df)
...
ggsave(file = "figs/combined_heatmap.png", width = 18, height = 10, dpi = 600)
...
dx_df <- rbind(SE_dx, USA_dx)

dx_combined <- ggplot(dx_df, aes(x = `Diagnosis Group`, y = value*100, fill = Cohort)) +
  geom_col(position = position_dodge()) +
  theme(legend.position = 'none') +
  theme(text = element_text(size = 16)) +
geom_text(aes(x = `Diagnosis Group`, y = (value * 100) + 5, label = paste0(round(value * 100, 0))), position = position_dodge(width = 0.9), size = rel(4)) + 
...
facet_wrap (~ `name`) +
  ...
  scale_fill_manual(values = c('SE' = '#1A4F66', 'USA' = "#EF6C45")) + 
...
Overall, SE: `r SE_results$manuscript_info$Compulsive_current_full[[1]]$valid.prc[[1]]`%; USA: `r USA_results$manuscript_info$Compulsive_current_full[[1]]$valid.prc[[1]]`% of the sample reported no history of maladaptive exercise, SE: `r SE_results$manuscript_info$Compulsive_current_full[[1]]$valid.prc[[2]]`%; USA: `r USA_results$manuscript_info$Compulsive_current_full[[1]]$valid.prc[[2]]`% reported a history of maladaptive exercise that was not current, and SE: `r SE_results$manuscript_info$Compulsive_current_full[[1]]$valid.prc[[3]]`%; USA: `r USA_results$manuscript_info$Compulsive_current_full[[1]]$valid.prc[[2]]`% reported current maladaptive exercise.
...
CET_combined <- rbind(SE_CET, USA_CET)
...
CET_combined_plot <- ggplot(CET_combined, aes(x = as.factor(ED100k_ex_compulsive_current2), y = Median, group = Cohort, color = Cohort)) +
  ...
  embarktools::embark_theme_a + 
    scale_color_manual(values = c('SE' = '#1A4F66', 'USA' = "#EF6C45")) +
...
EDEQ_combined <- rbind(SE_EDEQ, USA_EDEQ)
...
The proportion of individuals meeting the clinical cutoff for current compulsive exercise on the CET varied as expected by endorsement of historical and current endorsement of maladaptive exercise on the ED100k. Only SE: `r SE_results$manuscript_info$CET_ED100k_raw_table$Freq[1]`%; USA: `r USA_results$manuscript_info$CET_ED100k_raw_table$Freq[1]`% of individuals who reported no history of maladaptive exercise met current CET clinical cutoff, while SE: `r SE_results$manuscript_info$CET_ED100k_raw_table$Freq[2]`%; USA: `r USA_results$manuscript_info$CET_ED100k_raw_table$Freq[2]`% of those reporting history, but not current maladaptive exercise (#2 but not #8) met clinical cutoff on the CET, and SE: `r SE_results$manuscript_info$CET_ED100k_raw_table$Freq[3]`%; USA: `r USA_results$manuscript_info$CET_ED100k_raw_table$Freq[3]`% of individuals meeting current maladaptive exercise (#8) on the ED100k met clinical cutoff for maladaptive exercise on the CET. Using a multinomial logistic regression approach with 'No Maladaptive Exercise History' on the ED100k coded as the reference category, those reporting maladaptive exercise history, but not current maladaptive exercise had an odds ratio of SE: `r SE_results$manuscript_info$CET_ED100k_ORs$CET_ED100k_ORs[[2]]`; USA: `r USA_results$manuscript_info$CET_ED100k_ORs$CET_ED100k_ORs[[2]]` and those reporting current maladaptive exercise had an odds ratio of SE: `r SE_results$manuscript_info$CET_ED100k_ORs$CET_ED100k_ORs[[3]]`; USA: `r USA_results$manuscript_info$CET_ED100k_ORs$CET_ED100k_ORs[[3]]`, indicating significant convergent validity of reports of current maladaptive exercise on the ED100k and meeting clinical cutoff on the 24-item CET.
...
ED100k_items_table <- ED100k_items_table %>%
  select(Variable, Response, `Freq (USA)`, `Percent (USA)`, `Freq (Sweden)`, `Percent (Sweden)`)
...
dx_contrasts <- rbind(dx_contrasts_se, dx_contrasts_usa)
```
...
CET_ANOVA <- dplyr::bind_rows(SE_results$CET_ANOVA_df, USA_results$CET_ANOVA_df)
...
CET_Contrasts <- dplyr::bind_rows(SE_results$CET_contrast_file, USA_results$CET_contrast_file)
...
EDEQ_ZIN <- dplyr::bind_rows(SE_results$EDEQ_ZIN_table, USA_results$EDEQ_ZIN_table)
...
OCI_tidy_results <- rbind(oci_tidy_results_SE, oci_tidy_results_USA)
