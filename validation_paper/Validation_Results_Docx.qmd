...
load('tabs/Result_dfs_AU.RData')
load('tabs/Result_dfs_demographicsAU.RData')
AUS_results <- mget(ls())
save(AUS_results, file = "tabs/AU_results.RData")
rm(list = ls())
...
AUS_results$Heatmap_df$Country <- "AU"
USA_results$Heatmap_df$Country <- "US"
SE_results$Heatmap_df$Country <- "SE"
NZ_results$Heatmap_df$Country <- "NZ"
...
dx_df$`Diagnosis Group` <- dplyr::recode(dx_df$`Diagnosis Group`, 
                                   `BN-BED Mixed` = "BN-BED")  
...
facet_wrap (~ `name`) +
  ...
  scale_fill_manual(values = c('NZ' = '#C2B824', 'SE' = '#1A4F66', 'US' = "#EF6C45", 'AU' = '#A481C7')) + 
...
# recorde variable 'Mood Improve' to be 'Mood improvement' and 'Wt Control' to be 'Weight control' for plotting
CET_combined$variable <- gsub("Mood Improve", "Mood improvement", CET_combined$variable)
CET_combined$variable <- gsub("Wt Control", "Weight control", CET_combined$variable)
# Change 'Enjoyment' to be 'Lack of enjoyment' for plotting
CET_combined$variable <- gsub("Enjoyment", "Lack of enjoyment", CET_combined$variable)

# make variable a factor and put total at the end
CET_combined$variable <- factor(CET_combined$variable, levels = c("Mood improvement", "Weight control", "Lack of enjoyment", "Avoidance", "Rigidity", "Total"))
...
EDEQ_combined_plot <- ggplot(EDEQ_combined, aes(x = as.factor(ED100k_ex_compulsive_current2), y = Median, group = Cohort, color = Cohort)) +
  ...
  geom_point(size = 4, position = position_dodge(width = 0.75)) +  # Represents the median
  scale_x_discrete(labels = c('No Hx', 'Hx only', 'Current')) +
  labs(title = 'Number of driven exercise episodes in the past 28 days \n by ED100k.v3 maladaptive exercise status', 
       x = 'Maladaptive exercise [ED100k.v3]', 
       y = 'Number of driven exercise episodes [EDEQ]') + 
  theme(legend.position = 'right', 
        text = element_text(size = 18)) +
  embarktools::embark_theme_a + 
    scale_color_manual(values = c("NZ" = "#C2B824", 'SE' = '#1A4F66', 'US' = "#EF6C45", 'AU' = '#A481C7')) +
  # angle the x-axis labels 45 degrees
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
   theme(strip.background = element_rect(fill = "lightgrey")) 
...
knitr::include_graphics("figs/Q1_sensitivity_US.png")
...
knitr::include_graphics("figs/NPV_US.png")
```
...
knitr::include_graphics("figs/MPS_combinedUS.png")
```
